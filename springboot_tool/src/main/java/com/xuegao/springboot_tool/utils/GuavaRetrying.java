 ...
 * <br/> @ClassName：GuavaRetrying
 * <br/> @Description：
 * <br/> @author：xuegao
 * <br/> @date：2020/12/15 23:32
 */
public class GuavaRetrying {
    // 使用 guava-retrying 实现灵活的重试机制
    // 我们的后端业务系统可能会出现接口调用失败、网络拥塞超时、任务执行失败、系统错误等异常情况，
    // 需要进行重试操作。但某些场景下我们对重试有特殊要求，比如延迟重试、降频重试等，
    // 此时自己编写重试代码会很繁琐，在 Java 中，可以使用 guava-retrying 帮我们实现灵活的重试机制。
